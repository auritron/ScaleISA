...
namespace analyzer_mod {
    ...
    std::expected<void, Error> Analyzer::scout_lbl(instruction_mod::Inst& inst, std::unordered_map<std::string, size_t>& label_table) { 
        ...
        if (!first_token.has_value()) { //check if first token exists, should not happen ideally but still
            return std::unexpected(Error(SemErr::MissingOpCodeError, first_token->file_line, first_token->file_col));
        } else {
            switch (first_token->token_type) {
                ...
                case TT::Label:
                    if (label_too_long) {
                        return std::unexpected(Error(SemErr::LabelTooLongError, first_token->file_line, first_token->file_col)); //if extra operands apart from just the label
                    } else {
                        return std::visit(overload{
                            [this, &inst, &label_table, first_token](const std::string& val) -> std::expected<void, Error> {
                                auto [_, result] = label_table.insert(std::make_pair(val, priv_inst_count * 4));
                                if (!result) {
                                    return std::unexpected(Error(SemErr::LabelAlreadyExists, first_token->file_line, first_token->file_col));
                                }
                                inst.inst_type = instruction_mod::InstType::LBL;
                                return {};
                            },
                            [this, first_token](auto) -> std::expected<void, Error> {
                                panic::panic();
                                return std::unexpected(Error(SemErr::UnknownSemanticError, first_token->file_line, first_token->file_col));
                            }
                        }, first_token->value);
                    }
                    break;
    ...
    std::expected<void, Error> Analyzer::validate_token(const instruction_mod::Token& token, const std::unordered_map<std::string, size_t>& label_table) const {
        return std::visit(overload{
            [&token](OpCode) -> std::expected<void, Error> { //shouldn't happen ideally
                return std::unexpected(Error(SemErr::InvalidOpCodePosition, token.file_line, token.file_col));
            },
            [&token](int tkn_i) -> std::expected<void, Error> { 
                switch (token.token_type) {
                    case TT::Register:
                        if (tkn_i < 0 || tkn_i >= instruction_mod::Inst::MAX_REG_COUNT) {
                            return std::unexpected(Error(SemErr::RegisterOutOfRange, token.file_line, token.file_col));
                        } else {
                            return {};
                        }
                    case TT::Immediate:
                        if (tkn_i < 0 || tkn_i > instruction_mod::Inst::MAX_IMM_VAL) {
                            return std::unexpected(Error(SemErr::ImmediateOutOfRange, token.file_line, token.file_col));
                        } else {
                            return {};
                        }
                    case TT::Address:
                        if (tkn_i < 0 || tkn_i > instruction_mod::Inst::MAX_ADDR_VAL) {
                            return std::unexpected(Error(SemErr::AddressOutOfRange, token.file_line, token.file_col));
                        } else {
                            return {};
                        }
                    default:
                        panic::panic(); //unreachable
                        return std::unexpected(Error(SemErr::UnknownSemanticError, token.file_line, token.file_col));
                }
            },
            [this, &token, &label_table](const std::string& tkn_s) -> std::expected<void, Error> { 
                switch (token.token_type) {
                    case TT::Label:
                        if (label_table.contains(tkn_s)) {
                            return {};
                        }
                        return std::unexpected(Error(SemErr::UndefinedLabelError, token.file_line, token.file_col));
                    case TT::Variable:
    ...
    std::expected<void, Error> Analyzer::validate_opcode(instruction_mod::Inst& inst, instruction_mod::OpCode opcode, const std::unordered_map<std::string, size_t>& label_table) const {
        ...
        if (opc_pattern == instruction_fmt.end()) {
            ...
            for (int tkn{1}; tkn < instruction_mod::Inst::INST_SIZE; ++tkn) {
                ...
                if (cur_token.has_value() != cur_target.has_value()) { // one is null and the other is not
                    return std::unexpected(Error(SemErr::IncorrectOperandFmt, cur_token->file_line, cur_token->file_col));
        ...
        if (cur_inst_type == instruction_mod::instruction_types.end()) {
            panic::panic();
            return std::unexpected(Error(SemErr::UnknownSemanticError, 0, 0));
        } else {
    ...
    //analyzer
    std::expected<void, Error> Analyzer::analyze(instruction_mod::Inst& inst, const std::unordered_map<std::string, size_t>& label_table) const {
            ...
            switch(first_token->token_type) {   
                case TT::OpCode: {
                    auto is_matched = std::visit(overload{
                        [this, &inst, &label_table](instruction_mod::OpCode oc) -> std::expected<void, Error> {
                            return validate_opcode(inst, oc, label_table);
                        },
                        [first_token](auto a) -> std::expected<void, Error> { 
                            panic::panic(); 
                            return std::unexpected(Error(SemErr::UnknownSemanticError, first_token->file_line, first_token->file_col)); 
                        }, //shouldn't happen
                    }, first_token->value);
                ...
                case TT::Label: //intentionally empty, covered in scout
                    break;
                default:
                    return std::unexpected(Error(SemErr::IncorrectFirstToken, first_token->file_line, first_token->file_col));
                    break;
            }
        }
        return {};
